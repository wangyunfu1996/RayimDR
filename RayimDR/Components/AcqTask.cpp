...
void AcqTask::startAcq()
{
    ...
    qDebug() << "Stack configuration: stackedFrame=" << acqCondition.stackedFrame << ", need to collect"
             << totalStackFrames << "frames";
...
void AcqTask::onImageReceived(QSharedPointer<QImage> image, int idx, int grayValue)
{
    ...
    qDebug() << "Received frame" << nReceivedIdx
             ...
             << ", width:" << image->width() << ", height:" << image->height() << ", idxInDet:" << idx;

    if (acqCondition.stackedFrame > 0)
    {
        this->onProgressChanged(QString("第 %1 帧 叠加数据 %2/%3 已接收")
                                    .arg(nProcessedStacekd.load() + 1)
                                    .arg(AcqTaskManager::Instance().stackedImageList.size())
                                    .arg(acqCondition.stackedFrame + 1));
    }
...
QImage AcqTask::stackImages(const QVector<QImage>& images)
{
    ...
    qDebug() << "[性能] 开始叠加：" << count << "帧图像，尺寸：" << width << "x" << height << "，总像素："
             << totalPixels;

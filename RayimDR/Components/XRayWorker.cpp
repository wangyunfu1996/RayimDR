...
XRayWorker::XRayWorker(XRayManager* owner) : QObject(nullptr), owner_(owner) {}
...
void XRayWorker::publishStatus(const XRayStatus& status)
{
    ...
    QMetaObject::invokeMethod(
        owner_, [owner = owner_, status]() { owner->onWorkerStatusChanged(status); }, Qt::QueuedConnection);
}
...
void XRayWorker::connectToXRaySource()
{
    ...
    qDebug() << "开始连接射线源，"
             << "vendor: " << vendor << "model: " << model << "ip: " << ip << "port: " << port;

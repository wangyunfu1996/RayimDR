...
XImageHelper::XImageHelper(QObject* parent) : QObject(parent) {}
...
{
    return QColor::fromHsv(QRandomGenerator::global()->bounded(hue),       // 色调 0-359
...
bool XImageHelper::saveImageU16Raw(const QImage& image, const QString& filePath)
{
    ...
    try
    {
        ...
        for (int y = 0; y < h; ++y)
        {
            ...
            // 写入一行数据
            qint64 bytesWritten = file.write(reinterpret_cast<const char*>(lineData), w * sizeof(ushort));

            if (bytesWritten != w * sizeof(ushort))
            {
                qWarning() << "写入第" << y << "行时出错，预期写入:" << w * sizeof(ushort)
                           << "字节，实际写入:" << bytesWritten;
                file.close();
                return false;
            }
        }
...
bool XImageHelper::saveImagePNG(const QImage& image, const QString& filePath, int compressionLevel)
{
    ...
    if (success)
    {
        ...
        qDebug() << "PNG保存成功:" << filePath << "尺寸:" << image.size() << "格式:" << image.format()
                 << "大小:" << fileSize << "压缩级别:" << compressionLevel;
    }
    else
    {
        qWarning() << "saveImagePNG: 保存失败" << filePath << "错误:" << writer.errorString();
    }
...
bool XImageHelper::saveImageJPG(const QImage& image, const QString& filePath, int quality)
{
    ...
    {
        qint64 fileSize = QFileInfo(filePath).size();
        qDebug() << "JPG保存成功:" << filePath << "尺寸:" << grayImage.size() << "格式: 8位灰度"
                 << "大小:" << fileSize << "质量:" << quality;
    }
    else
    {
        qWarning() << "saveImageJPG: 保存失败" << filePath << "错误:" << writer.errorString();
    }
...
void XImageHelper::testOpencv(const QString& filePath, int width, int height)
{
    ...
    auto display16BitImage = [](const cv::Mat& data)
    {
        ...
        {
            // 线性拉伸到0-255
            data.convertTo(displayImage, CV_8UC1, 255.0 / (maxVal - minVal), -255.0 * minVal / (maxVal - minVal));
        }
        else
        {
            // 如果所有值相同

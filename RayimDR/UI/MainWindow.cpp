	...
	: ElaWindow(parent)
{
	...
	connect(&xSignaHelper, &XSignalsHelper::signalShowErrorMessageBar, this, [this](const QString& msg) {
		ElaMessageBar::error(ElaMessageBarType::TopRight, "错误", msg, 4000);
		});
	connect(&xSignaHelper, &XSignalsHelper::signalShowSuccessMessageBar, this, [this](const QString& msg) {
	...
	connect(_XGraphicsView, &XGraphicsView::signalSrcU16ImageListSizeChanged, _XImageAdjustTool, &XImageAdjustTool::updateIdxRange);
	connect(_XGraphicsView, &XGraphicsView::signalMinMaxValueChanged, _XImageAdjustTool, &XImageAdjustTool::updateWLRange);
...
void MainWindow::onAcqImageReceived(AcqCondition condition, int receivedIdx)
{
	...
	if (condition.acqType == AcqType::DR)
	{
		...
		else
		{
			...
			updateStatusText(QString("多张DR采集，当前接受帧数：%1，共 %2 帧").arg(receivedIdx).arg(condition.frame));
		}
	}
}

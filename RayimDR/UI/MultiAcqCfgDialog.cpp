	...
	: ElaDialog(parent)
{
	...
	connect(ui.pushButton_select, &QPushButton::clicked, this, [this]() {
		// 选择文件夹
		QString folderPath = QFileDialog::getExistingDirectory(nullptr, "选择保存文件夹", ".", QFileDialog::ShowDirsOnly);
...
bool MultiAcqCfgDialog::getCfg(int& n, bool& saveToFiles, QString& savePath)
{
	...
	if (ui.checkBox_saveToFiles->isChecked())
	{
		if (ui.lineEdit_savePath->text().isEmpty() ||
			ui.lineEdit_savePath->text().isNull() ||
